					...
					while ((stringiterator = fileReader.readLine()) != null) {
						...
						if (entity.getPersistentData().getDouble("process") < entity.getPersistentData().getDouble("line_count")) {
							if (entity.getPersistentData().getDouble("process") >= entity.getPersistentData().getDouble("process_save")
									&& (entity.getPersistentData().getDouble("process") < entity.getPersistentData().getDouble("process_save") + ThtModVariables.MapVariables.get(world).rt_dynamic_process_limit
											|| ThtModVariables.MapVariables.get(world).rt_dynamic_process_limit == 0)) {
								if (stringiterator.startsWith("+")) {
									if (stringiterator.contains("@tr") || stringiterator.contains("@br") || stringiterator.contains("@tw") || stringiterator.contains("@lt") || stringiterator.contains("@le")) {
										...
										if (!stringiterator.contains("@le")) {
											if ((stringiterator).equals("+s^0^0^0@tro")) {
												{
													Entity _ent = entity;
											...
											if (ForgeRegistries.BLOCKS.getValue(new ResourceLocation(((block.replace("[persistent=true]", ""))).toLowerCase(java.util.Locale.ENGLISH))).defaultBlockState()
												...
												if (!("").equals("Leaves Regrow")) {
													if (entity.getPersistentData().getBoolean("dead_tree") == false
															&& ((ForgeRegistries.BLOCKS.getValue(new ResourceLocation(((block.replace("[persistent=true]", ""))).toLowerCase(java.util.Locale.ENGLISH))).defaultBlockState()
																	...
																	&& world.getBiome(BlockPos.containing(x, y, z)).is(TagKey.create(Registries.BIOME, new ResourceLocation("tht:no_snow_biomes")))
																	|| !(ForgeRegistries.BLOCKS.getValue(new ResourceLocation(((block.replace("[persistent=true]", ""))).toLowerCase(java.util.Locale.ENGLISH))).defaultBlockState()
																			.is(BlockTags.create(new ResourceLocation("tht:deciduous_leaves_blocks")))
																			|| ForgeRegistries.BLOCKS.getValue(new ResourceLocation(((block.replace("[persistent=true]", ""))).toLowerCase(java.util.Locale.ENGLISH))).defaultBlockState()
																		...
																		if (!_ent.level().isClientSide() && _ent.getServer() != null) {
																			_ent.getServer().getCommands().performPrefixedCommand(
																					...
																					(("execute positioned " + pos + " if score leaves_drop THT matches .."
																							...
																							+ "summon armor_stand ~ ~ ~ {CustomName:'{\"text\":\"THT-leaves_drop\"}',Invisible:1b,Marker:1b,ArmorItems:[{},{},{},{id:\""
																							+ block.replace("[persistent=true]", "") + "\",Count:1b}],ForgeData:{block:\"" + block + "\"}}"));
																		}
																	}
																} else {
																	{
							...
							} else if (entity.getPersistentData().getDouble("process") >= entity.getPersistentData().getDouble("process_save") + ThtModVariables.MapVariables.get(world).rt_dynamic_process_limit) {
