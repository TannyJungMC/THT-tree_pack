...
public class RandomTreeTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (!((world.getBlockState(BlockPos.containing(x, y - 1, z))).getBlock() == Blocks.COMMAND_BLOCK || (world.getBlockState(BlockPos.containing(x, y - 1, z))).getBlock() == Blocks.STRUCTURE_BLOCK)) {
			if (entity.getPersistentData().getBoolean("start_tp") == false) {
				...
				if (ThtModVariables.MapVariables.get(world).auto_gen == false) {
					...
					if (ThtModVariables.MapVariables.get(world).grid_fix == true && entity.getPersistentData().getBoolean("grid_fix") == true
							&& !(world.getBlockState(BlockPos.containing(variable_number, y, variable_number2))).is(BlockTags.create(new ResourceLocation("tht:air_blocks")))
							&& !(world.getBlockState(BlockPos.containing(x, y - 1, z))).is(BlockTags.create(new ResourceLocation("tht:air_blocks")))) {
						{
							Entity _ent = entity;
							if (!_ent.level().isClientSide() && _ent.getServer() != null) {
