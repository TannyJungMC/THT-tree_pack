		...
		if (ThtModVariables.MapVariables.get(world).loop_second < 20) {
			...
			if (!("").equals("Tree Placer")) {
				if (ThtModVariables.MapVariables.get(world).tree_placer_auto_speed == true) {
					...
					ThtModVariables.MapVariables.get(world).syncData(world);
					if (true) {
						if (ThtModVariables.MapVariables.get(world).tree_placer_auto_speed_test > 100) {
							ThtModVariables.MapVariables.get(world).tree_placer_process_limit = ThtModVariables.MapVariables.get(world).tree_placer_process_limit - 100;
							ThtModVariables.MapVariables.get(world).syncData(world);
						} else if (ThtModVariables.MapVariables.get(world).tree_placer_auto_speed_test > 20) {
							ThtModVariables.MapVariables.get(world).tree_placer_process_limit = ThtModVariables.MapVariables.get(world).tree_placer_process_limit - 10;
							ThtModVariables.MapVariables.get(world).syncData(world);
						} else {
							ThtModVariables.MapVariables.get(world).tree_placer_process_limit = ThtModVariables.MapVariables.get(world).tree_placer_process_limit + 10;
							ThtModVariables.MapVariables.get(world).syncData(world);
						}
						if (ThtModVariables.MapVariables.get(world).tree_placer_process_limit < 10) {
							ThtModVariables.MapVariables.get(world).tree_placer_process_limit = 10;
							ThtModVariables.MapVariables.get(world).syncData(world);
						} else if (ThtModVariables.MapVariables.get(world).tree_placer_process_limit > 100) {
							ThtModVariables.MapVariables.get(world).tree_placer_process_limit = 100;
							ThtModVariables.MapVariables.get(world).syncData(world);
						}
					}
					if (ThtModVariables.MapVariables.get(world).tree_placer_auto_speed_test == 100) {
						...
						ThtModVariables.MapVariables.get(world).syncData(world);
					} else if (ThtModVariables.MapVariables.get(world).tree_placer_auto_speed_test == 10) {
						ThtModVariables.MapVariables.get(world).tree_placer_tick_set = 10;
						ThtModVariables.MapVariables.get(world).syncData(world);
					} else {
						ThtModVariables.MapVariables.get(world).tree_placer_tick_set = 5;
						ThtModVariables.MapVariables.get(world).syncData(world);
					}
					ThtModVariables.MapVariables.get(world).tree_placer_count_limit = Math.ceil(ThtModVariables.MapVariables.get(world).tree_placer_process_limit / 10);
					ThtModVariables.MapVariables.get(world).syncData(world);
					if (ThtModVariables.MapVariables.get(world).developer_mode == true) {
						if (world instanceof ServerLevel _level)
							_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
		...
		if (!("").equals("Tree Placer")) {
			if (ThtModVariables.MapVariables.get(world).tree_placer_tick_set > 0) {
				if (ThtModVariables.MapVariables.get(world).tree_placer_tick < ThtModVariables.MapVariables.get(world).tree_placer_tick_set) {
					...
					if (true) {
						variable_text = "execute at @p as @e[name=THT-world_gen,nbt={ForgeData:{world_gen_chosen:1b}}";
						...
						if (ThtModVariables.MapVariables.get(world).version_1192 == true) {
							variable_text = variable_text + "] at @s run THT dev tree_placer";
						} else {
							variable_text = variable_text + "] at @s if loaded ~ ~ ~ run THT dev tree_placer";
						}
						if (world instanceof ServerLevel _level)
							_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
									variable_text);
